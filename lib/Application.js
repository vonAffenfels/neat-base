...
module.exports = class Application {
    ...
    static loadScriptConfig(scriptName) {
    ...
    static runScript(packageData) {
        ...
        this.loadApplicationConfigs().then(() => {
            ...
            if (Application.modules.irapi) {
                mockRequest = new MockExpressRequest(reqOptions);

                if (!Application.modules.irfrontend) {
                    i18n.configure({
                        locales: Application.appConfigs.misc.locales,
                        directory: Application.config.config_path + '/locales',
                        defaultLocale: Application.appConfigs.misc.defaultLocale,
                        updateFiles: false,
                        indent: "    ",
                        extension: ".json",
                        objectNotation: true,
                ...
                i18n.init(mockRequest, {});

                return Application.modules.irapi.setClient(packageData.scriptConfig.client || "desktop-website", mockRequest);
            } else {
            ...
            var scriptConfig = this.loadScriptConfig(this.scriptName, packageData);

            try {
                var scriptClass = require(mainScriptFile);
                scriptInstance = new scriptClass(this.scriptName, scriptConfig, packageData.options, packageData.arguments);
                scriptInstance.req = mockRequest;
                scriptInstance.reqOptions = reqOptions
