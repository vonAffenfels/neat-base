...
module.exports = class Tools {
     ...
     *
     * @param err
     * @param translator req.__
     * @returns {{}}
     */
    static formatMongooseError(err, translator) {
        translator = translator || function (msg) {
                return msg;
            };
        let formattedError = {};
        ...
        for (let field in err.errors) {
            if (err.errors[field].errors) {
                let subErrors = Tools.formatMongooseError(err.errors[field]);
                for (let subField in subErrors) {
                    formattedError[subField] = translator(subErrors[subField]);
                }
            } else {
                formattedError[field] = translator(err.errors[field].message);
            }
        }
